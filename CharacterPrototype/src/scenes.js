...
// Distant landforms, sunk so only their caps show. They exist to stop the
// horizon being a perfectly flat line, which reads as a void rather than as
// somewhere.
function hillRing(random, count, color, radius, size, sink) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const hill = new THREE.Mesh(new THREE.SphereGeometry(size + random() * size * 0.7, 12, 8), material);
...
function buildPark() {
  ...
  const ground = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ map: tiled(grassTexture(), 42), roughness: 1 })
  );
  ...
  for (let i = 0; i < 34; i++) {
    ...
    const tree = coneTree(random, 0x6b4a30, 0x3f7a3a);
  ...
  group.add(hillRing(random, 10, 0x5c8f52, 48, 8, 6));
...
function pond(x, z, radius, random) {
  ...
  for (let i = 0; i < 24; i++) {
    const angle = random() * Math.PI * 2;
    const reed = new THREE.Mesh(new THREE.ConeGeometry(0.05, 0.7 + random() * 0.5, 4), reedMat);
    reed.position.set(
...
// ---- 海辺 ----
// Sand underfoot, water in every direction past it. The point of this one is
// the light: a low sun over water is the hardest and best thing to photograph
// a face against, and the sea gives it a second, bigger source bouncing up.
function buildBeach() {
  const random = makeRandom(1071);
  const group = new THREE.Group();
  const sway = [];

  // Nine metres, not seventeen. At seventeen the water was a stripe on the
  // horizon and the scene was a sandpit: you cannot photograph somebody
  // against the sea if the sea is past the fog.
  const SHORE = 9;

  const sand = new THREE.Mesh(
    new THREE.CircleGeometry(SHORE + 1.2, 56),
    new THREE.MeshStandardMaterial({ map: tiled(sandTexture(), 16), roughness: 1 })
  );
  ...
  const wet = new THREE.Mesh(
    new THREE.RingGeometry(SHORE - 1.6, SHORE + 1.2, 56),
    new THREE.MeshStandardMaterial({ map: tiled(wetSandTexture(), 12), roughness: 0.55 })
  );
  wet.rotation.x = -Math.PI / 2;
  wet.position.y = 0.012;
  group.add(wet);

  // The sea, as one big disc with vertex colours: turquoise where it is
  // shallow, deep blue by the horizon. A single flat colour out to the fog
  // reads as painted card.
  const seaGeo = new THREE.RingGeometry(SHORE - 2, 220, 72, 6);
  const shallow = new THREE.Color(0x49b0bd);
  const deep = new THREE.Color(0x1b4f7a);
  const colors = [];
  const position = seaGeo.attributes.position;
  for (let i = 0; i < position.count; i++) {
    const radius = Math.hypot(position.getX(i), position.getY(i));
    const t = THREE.MathUtils.clamp((radius - SHORE) / 48, 0, 1);
  ...
  // Two rings of surf that run up the sand and back. Animated by main.js
  // through userData.surf -- the sea being the only still thing on a beach
  // is the giveaway that it is geometry.
  for (let i = 0; i < 2; i++) {
    const foam = new THREE.Mesh(
      new THREE.RingGeometry(SHORE - 1.5, SHORE - 0.95, 64),
      new THREE.MeshStandardMaterial({
  ...
  for (let i = 0; i < 9; i++) {
    ...
    const rock = new THREE.Mesh(new THREE.DodecahedronGeometry(0.3 + random() * 0.75, 0), rockMat);
  ...
  // A few wind-bent palms, back from the water.
  for (let i = 0; i < 7; i++) {
    const angle = random() * Math.PI * 2;
  ...
  group.add(hillRing(random, 5, 0x4d6f57, 96, 14, 9));
...
function jetty() {
  ...
  const LENGTH = 11;
  for (let i = 0; i < 26; i++) {
    const plank = new THREE.Mesh(new THREE.BoxGeometry(1.7, 0.08, 0.34), wood);
  ...
  for (let i = 0; i < 5; i++) {
    for (const side of [-1, 1]) {
      const pile = new THREE.Mesh(new THREE.CylinderGeometry(0.09, 0.09, 1.4, 6), dark);
...
function palmTree(random) {
  ...
  const trunk = new THREE.Mesh(
    new THREE.CylinderGeometry(0.11, 0.18, 3.4, 7),
  ...
  for (let i = 0; i < 7; i++) {
    const frond = new THREE.Mesh(new THREE.ConeGeometry(0.3, 1.9, 4), frondMat);
...
function buildStreet() {
  ...
  const ground = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ map: tiled(asphaltTexture(), 40), roughness: 1 })
  );
  ground.rotation.x = -Math.PI / 2;
  ground.receiveShadow = true;
  group.add(ground);
  ...
  for (const side of [-1, 1]) {
    const pavement = new THREE.Mesh(
      new THREE.BoxGeometry(5.5, KERB, 120),
      new THREE.MeshStandardMaterial({ map: tiled(pavementTexture(), 2, 44), roughness: 1 })
    );
  ...
  for (let i = -18; i <= 18; i++) {
    const dash = new THREE.Mesh(new THREE.PlaneGeometry(0.16, 1.6), lineMat);
  ...
  for (let i = 0; i < 8; i++) {
    const stripe = new THREE.Mesh(new THREE.PlaneGeometry(0.45, ROAD_HALF * 2 - 0.3), lineMat);
  ...
  for (const side of [-1, 1]) {
    ...
    while (z < 46) {
      ...
      for (const pane of building.userData.windows) {
        // Two thirds lit, not all of them: a block with every light burning
        // reads as a film set rather than as a street where people live.
        if (random() > 0.34) {
          nightGlow.push({
            material: pane.material,
            color: [0xffd9a0, 0xffe9c4, 0xcfe0ff][Math.floor(random() * 3)],
  ...
  for (const side of [-1, 1]) {
    for (let i = -2; i <= 2; i++) {
      const tree = coneTree(random, 0x5b4632, 0x4d7a45);
...
function shopfront(width, height, depth, color, random) {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < columns; c++) {
      // Its own material per pane, so night can light some and not others.
      // Shared, they would all come on together.
      const paneMat = new THREE.MeshStandardMaterial({
        color: 0x2b3a48, roughness: 0.18, metalness: 0.55,
      });
      const pane = new THREE.Mesh(new THREE.PlaneGeometry(0.95, 1.25), paneMat);
      pane.position.set(
